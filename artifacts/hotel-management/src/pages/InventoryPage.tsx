...
export default function InventoryPage() {
  ...
  return (
    <div className="page-container">
      ...
      {loading ? (
        <div className="space-y-2">{[...Array(6)].map((_, i) => <div key={i} className="h-16 bg-white dark:bg-slate-800 rounded-xl animate-pulse" />)}</div>
      ) : displayed.length === 0 ? (
              ...
              {displayed.map((item, i) => {
                ...
                return (
                  <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                    ...
                    <td className="text-center font-semibold text-slate-900 dark:text-slate-200">{item.quantity} {item.unit}</td>
                    <td className="text-center text-slate-500">{item.minStock} {item.unit}</td>
                    <td className="text-right text-slate-700 dark:text-slate-300">₹{item.costPerUnit}</td>
                    ...
                    <td className="text-center">
                      <button onClick={() => openEdit(item)} className="text-slate-500 hover:text-slate-900 dark:text-slate-200 p-1.5 rounded-lg hover:bg-slate-200/50 dark:bg-slate-700/50">
                        <Edit2 size={14} />
                      </button>
                    </td>
                  </motion.tr>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <h3 className="font-bold text-slate-900 dark:text-slate-100 mb-5">{editing ? "Update Stock" : "Add Inventory Item"}</h3>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1.5">Unit</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1.5">Min Stock Alert</label>
                  <input type="number" value={form.minStock} onChange={e => setForm(f => ({ ...f, minStock: e.target.value }))} className="input-dark" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1.5">Cost per Unit (₹)</label>
                  <input type="number" value={form.costPerUnit} onChange={e => setForm(f => ({ ...f, costPerUnit: e.target.value }))} className="input-dark" />
                </div>
                <div>
                  <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1.5">Supplier</label>
