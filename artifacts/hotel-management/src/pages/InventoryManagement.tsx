    ...
    <DashboardLayout title="Inventory Management">
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button onClick={openCreate} className="rounded-xl h-9 font-semibold" data-testid="button-add-item">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-secondary/40">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wide hidden sm:table-cell">Level</th>
                      ...
                      <td className="px-4 py-3 hidden sm:table-cell">
                        <div className="flex items-center gap-2 max-w-[120px]">
                          <div className="flex-1 h-1.5 rounded-full bg-secondary overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all ${pct <= 33 ? "bg-rose-500" : pct <= 66 ? "bg-amber-500" : "bg-emerald-500"}`}
                              style={{ width: `${pct}%` }}
                            />
                          </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-sm rounded-2xl">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5"><Label className="font-semibold text-sm">Name *</Label><Input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="Chicken" className="rounded-xl" data-testid="input-name" /></div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5"><Label className="font-semibold text-sm">Quantity *</Label><Input type="number" min="0" step="0.1" value={form.quantity} onChange={(e) => setForm((p) => ({ ...p, quantity: e.target.value }))} placeholder="10" className="rounded-xl" data-testid="input-quantity" /></div>
              <div className="space-y-1.5"><Label className="font-semibold text-sm">Unit *</Label><Input value={form.unit} onChange={(e) => setForm((p) => ({ ...p, unit: e.target.value }))} placeholder="kg" className="rounded-xl" data-testid="input-unit" /></div>
            </div>
            <div className="space-y-1.5"><Label className="font-semibold text-sm">Low Stock Threshold *</Label><Input type="number" min="0" step="0.1" value={form.lowStockThreshold} onChange={(e) => setForm((p) => ({ ...p, lowStockThreshold: e.target.value }))} placeholder="5" className="rounded-xl" data-testid="input-threshold" /></div>
          </div>
          <DialogFooter className="gap-2">
            <Button variant="outline" className="rounded-xl" onClick={() => setDialogOpen(false)}>Cancel</Button>
            <Button className="rounded-xl font-semibold" onClick={handleSubmit} disabled={createItem.isPending || updateItem.isPending} data-testid="button-save">{editing ? "Save Changes" : "Add Item"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </DashboardLayout>
