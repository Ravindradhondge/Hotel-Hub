    ...
    <DashboardLayout title="Menu Management">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          <div className="flex gap-2 flex-1 flex-wrap">
            <div className="relative flex-1 min-w-[160px] max-w-xs">
              ...
              <Input className="pl-9 h-9 rounded-xl" placeholder="Search items…" value={search} onChange={(e) => setSearch(e.target.value)} data-testid="input-search" />
            </div>
            {/* Category chips */}
          ...
          <Button onClick={openCreate} className="rounded-xl h-9 font-semibold" data-testid="button-create-item">
              ...
              <tbody className="divide-y divide-border">
                {isLoading ? (
                  ...
                  <tr key={item.id} data-testid={`row-item-${item.id}`} className="hover:bg-secondary/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-secondary text-secondary-foreground">{item.category}</span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Button variant="ghost" size="icon" className="w-8 h-8 rounded-lg" onClick={() => openEdit(item)} data-testid={`button-edit-${item.id}`}><Pencil className="w-3.5 h-3.5" /></Button>
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="ghost" size="icon" className="w-8 h-8 rounded-lg text-destructive hover:text-destructive" data-testid={`button-delete-${item.id}`}><Trash2 className="w-3.5 h-3.5" /></Button>
                          </AlertDialogTrigger>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md rounded-2xl">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5"><Label className="font-semibold text-sm">Name *</Label><Input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="Butter Chicken" className="rounded-xl" data-testid="input-name" /></div>
            <div className="space-y-1.5">
              ...
              <Select value={customCategory ? "__custom__" : form.category} onValueChange={(v) => { if (v === "__custom__") return; setForm((p) => ({ ...p, category: v })); setCustomCategory(""); }}>
                <SelectTrigger className="rounded-xl" data-testid="select-category"><SelectValue placeholder="Select category" /></SelectTrigger>
                <SelectContent className="rounded-xl">
                  {categories.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                  <SelectItem value="__custom__">+ New category</SelectItem>
                </SelectContent>
              </Select>
              <Input placeholder="Or type new category…" value={customCategory} onChange={(e) => setCustomCategory(e.target.value)} className="rounded-xl mt-1" data-testid="input-custom-category" />
            </div>
            <div className="space-y-1.5"><Label className="font-semibold text-sm">Price (₹) *</Label><Input type="number" min="0" step="0.01" value={form.price} onChange={(e) => setForm((p) => ({ ...p, price: e.target.value }))} placeholder="250" className="rounded-xl" data-testid="input-price" /></div>
            <div className="space-y-1.5"><Label className="font-semibold text-sm">Description</Label><Textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder="Optional description…" className="resize-none h-20 rounded-xl" data-testid="input-description" /></div>
            <div className="flex items-center gap-3 bg-secondary/40 rounded-xl px-4 py-3">
              <Switch checked={form.available} onCheckedChange={(v) => setForm((p) => ({ ...p, available: v }))} data-testid="switch-available-form" />
          ...
          <DialogFooter className="gap-2">
            ...
            <Button className="rounded-xl font-semibold" onClick={handleSubmit} disabled={createItem.isPending || updateItem.isPending} data-testid="button-save-item">
