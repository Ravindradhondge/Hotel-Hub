        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={card.label} className={`bg-card border border-border rounded-2xl p-4 card-shadow ring-2 ${card.ring} relative overflow-hidden`}>
              ...
              <div className="text-2xl font-extrabold">{card.count}</div>
              <div className="text-xs font-semibold text-muted-foreground mt-1">{card.label} Today</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-1.5 bg-secondary p-1 rounded-xl w-fit">
          {(["today", "history"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all capitalize ${tab === t ? "bg-card shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
          ...
          todayLoading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {todayData.map((s) => {
                ...
                return (
                  <div key={s.userId} className={`border rounded-2xl p-4 card-shadow ${st.bg} ${st.border} flex flex-col gap-2`}>
                    <div className="flex items-start justify-between">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-extrabold text-sm ${ROLE_STYLE[s.userRole] ?? "bg-secondary text-primary"}`}>
